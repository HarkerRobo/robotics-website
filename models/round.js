...
roundSchema.statics.setRoundAwatingScouts = function(tournament_id, round) {
  return new Promise((resolve, reject) => {
    this.updateMany(
      { tournament: tournament_id, number: { $ne: round }, stage: { $gt: round.not_started } },
      { stage: stages.ended }
    )
    .then(() => {
      this.findOneAndUpdate({ tournament: tournament_id, number: round }, { stage: stages.awaiting_scouts })
      .then(resolve)
      .catch(reject)
    })
    .catch(reject)
...
roundSchema.statics.setRoundStarted = function(tournament_id, round) {
  return new Promise((resolve, reject) => {
    ...
    .then(() => {
      this.findOneAndUpdate({ tournament: tournament_id, number: round }, { stage: stages.started })
      .then(resolve)
      .catch(reject)
    })
    .catch(reject)
...
roundSchema.statics.getCurrentRound = function(tournament_id) {
  return this.findOne({ tournament: tournament_id, stage: { $gt: round.not_started, $lt: round.ended } })
}
