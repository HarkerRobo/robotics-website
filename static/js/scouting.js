...
async function cfetch(url, options) {
    ...
    try {
        ...
        try {
            ...
            throw new NiceError(
                text.match("<p>(.*?)</p>")[1].replace(/<\/?strong>/g, "")
            );
        }
    } catch (e) {
...
function finalize(data) {
    ...
    document.getElementById("formsub").addEventListener("click", (e) => {
        e.preventDefault();
        var crossed = JSON.parse(
            document.querySelector('input[name="crossedline"]:checked').value
        );
        var endedplat = JSON.parse(
            document.querySelector('input[name="endedplat"]:checked').value
        );
        var lifted = JSON.parse(
            document.querySelector('input[name="lifted"]:checked').value
        );
        ...
        cfetch("/member/scouting/upload", {
            ...
            (e) => {
                console.error(e);
                alert(
                    e instanceof NiceError
                        ? e.message
                        : "Scouting API Error! Yay!"
                );
            }
        );
    });
...
document.getElementById("promptsub").addEventListener("click", () => {
    enter(Number(document.getElementById("promptel").value)).catch((e) => {
        console.error(e);
        if (e.status == 409) {
