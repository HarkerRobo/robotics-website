...
$($root).bind(
    "scroll mousedown wheel DOMMouseScroll mousewheel keyup wheel",
    function (e) {
        console.log(e.type);
        if (
            e.which > 0 ||
...
function getCurrentTile() {
    ...
    tiles.each(function (index, thisTile) {
        ...
        if (thisTilePosition <= scrollPosition) res = thisTile;
    });
    return res;
}
...
function snapToClosest() {
    ...
    tiles.each(function (index, curTile) {
        var distToClosest = Math.abs(
            $(closestTile).offset().top - scrollPosition
        );
