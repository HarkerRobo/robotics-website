...
|-------------------------|-------------------------------------------------|-------------
| GoogleClientID          | "[GoogleClientID].apps.googleusercontent.com"   | The google client ID used for "sign in with Google". Ask for it from the webmaster if needed. Only needed if `runInternal` is set to `true`.
