                            ...
                            if (result.statusCode === 200) {
                                if (data.aud === config.GoogleClientID) {
                                    if (
                                        ...
                                        if (
                                            req.app.locals.admins.includes(
                                    ...
                                    req.session.auth = { loggedin: false };
                                    res.status(400).end(
                                        "Token does match Google Client ID"
                                    );
                                }
                            } else {
                                req.session.auth = { loggedin: false };
...
router.all("/*", function (req, res, next) {
    if (req.session.auth.level >= 1) {
        ...
        res.render("pages/member/error", {
            statusCode: 401,
            error: "Must be authenticated as harker student.",
        });
    }
});
    ...
    Purchase.create(
        {
            ...
            shipping_and_handling: toDollarAmount(
                req.body.shipping_and_handling,
                0
            ),
            ...
            transporter.sendMail(
                {
                    ...
                    text:
                        "Purchase Order can be found here: https://robodev.harker.org/member/purchase/view/" +
                        req.params.purchase_id, // plaintext body
                },
                (err) => {
                    console.error(err);
                }
...
router.get("/purchase/edit/:purchase_id", function (req, res) {
    Purchase.findById(req.params.purchase_id, (err, purchase) => {
        ...
        else if (purchase.submitted_by === req.session.auth.info.email)
