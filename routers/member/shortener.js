...
router.all('/*', function (req, res, next) {
    if (req.auth.level >= ranks.director) { 
      next()
        ...
        res.render('pages/member/error', { statusCode: 401, error: "Must be a director to use the URL shortener."})
...
router.get("/urls", async (req, res) => {
  res.json(await Url.find({}, {path: 1, url: 1, uses: 1, _id: 0}));
});

router.post("/delete", async (req, res) => {
  if(!req.body.short)
    return res.json({error: "Specify short URL"}).end();
  const url = await Url.findOne({path: req.body.short});
  if(!url)
    return res.json({error: "Short URL does not exist."}).end();
