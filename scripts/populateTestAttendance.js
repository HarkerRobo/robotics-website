...
(async function() {
    ...
    while(totalEntries--) {
        await Entry.create({
            email: Math.random().toString(36).substring(2) + "@students.harker.org",
...
function getToday() {
    return new Date(new Date().getFullYear(), new Date().getMonth(), new Date().getDay() - parseInt(Math.random() * 5));
}